 ...
 *********************/
 ...
 module.exports = {
    ...
    listSpotify: function  (collectionIdent, daylimit, limit, offset, callback) {
        ...
        this.mapReduce(collectionIdent, urlMap, urlReduce, "spotify", daylimit, limit, offset, callback);
    ...
    listGithub: function  (collectionIdent, daylimit, limit, offset, callback) {
        ...
        var urlReduce = function(key, values) { //reduce function
            ...
            for (var i in values) {
                ...
                if( reduced.data.forks < values[i].data.forks 
                    || reduced.data.watchers < values[i].data.watchers ) {
                        reduced.data.forks = values[i].data.forks;
                    reduced.data.watchers = values[i].data.watchers;
                }
        ...
        this.mapReduce(collectionIdent, urlMap, urlReduce, "github", daylimit, limit, offset, callback);

    },

    mapReduce: function (collectionIdent, urlMap, urlReduce, type, daylimit, limit, offset, callback) {
        mongoose.connection.db.collection(collectionIdent, function (err, collection) {
            ...
            var query = {'date' : { $gt: now }, 'type' : type};
